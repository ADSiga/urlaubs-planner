      ...
      <div className="flex items-center gap-2">
        <div className="flex flex-col items-end">
          ...
          <span className="text-[8px] font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mt-0.5">
            {ROLE_LABELS[principalRole] ?? principalRole}
          </span>
        </div>
        <button
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="w-full max-w-xs rounded-3xl bg-white p-8 shadow-2xl dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 scale-in-center">
            <div className="mb-6 text-center">
              ...
              <h3 className="text-xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight">Anmelden</h3>
              <p className="mt-1 text-xs font-medium text-zinc-400">Mitglied oder Personal</p>
            </div>

            {/* Tab switcher */}
            <div className="flex rounded-2xl bg-zinc-100 dark:bg-zinc-800 p-1 mb-6 gap-1">
              <button
                type="button"
                onClick={() => handleTabChange("member")}
                className={`flex-1 rounded-xl py-2 text-xs font-bold transition-all ${tab === "member" ? "bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-50 shadow-sm" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700"}`}
              >
                Mitglied
              </button>
              <button
                ...
                className={`flex-1 rounded-xl py-2 text-xs font-bold transition-all ${tab === "staff" ? "bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-50 shadow-sm" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700"}`}
              >
                Personal
              </button>
            </div>
              ...
              <form onSubmit={handleMemberSubmit} className="space-y-4">
                <div>
                  <input
                    ...
                    placeholder="E-Mail"
                    className={`w-full rounded-2xl border-2 ${error ? "border-red-500 bg-red-50 dark:bg-red-950/20" : "border-zinc-100 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 focus:border-emerald-500"} px-4 py-3 text-sm outline-none transition-all`}
                    autoFocus
                    autoComplete="email"
                  />
                </div>
